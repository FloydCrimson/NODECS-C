...
const configuration = argv[1] || 'prod';
...
for (const project in parentConfigJSON.projects) {
    ...
    execSync(['ng', 'build', project, '--configuration=' + configuration].join(' '), { stdio: 'inherit', cwd: parentDirectory });
    console.log('[project-builder] built.');
    // PACK
    console.log('[project-builder] packing...');
    const childReleaseDirectory = path.join(parentReleaseDirectory, parentConfigJSON.projects[project].root.replace(parentConfigJSON.newProjectRoot, ''));
